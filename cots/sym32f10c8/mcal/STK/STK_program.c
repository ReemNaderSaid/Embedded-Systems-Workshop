...
Std_ReturnType STK_SetBusyWait(u32 Copy_Microseconds)
{
   ...
   /**< other case if no. of ticks is more han the maximum delay*/

    else if(TickRequired>0x00FFFFFF){

    /**< chek whether the number of ticks is still greater than the maximum delay time*/
        while (TickRequired>0x00FFFFFF)
        {
        /**< Set the reload value of the SysTick timer as the maximum delay time */
            STK->LOAD=0x00FFFFFF;
        ...
        /**< subtract the no, of ticks given from the maximum delay time to know the remaining no. of ticks that needs to be executed*/
            TickRequired-=0x00FFFFFF;
        }
        /**< check if the no. of ticks became equal to zero*/
        if (TickRequired==0)

        Local_FunctionStatus = E_OK;


        /**< if the no. of ticks remained is not equal to zero and is less than the maximum delay time */
         else
    { 
          /**< Set the reload value of the SysTick timer */ 
        STK->LOAD = TicksRequired;
